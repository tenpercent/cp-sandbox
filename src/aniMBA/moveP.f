...
C ... for nonlinear minimization procedure
      Real*8  NLnFnc, Z(3, 3), U(3), U1(3)

      Real*8  distSR, distSF
      Real*8  q(4), qMin, hMin, hMax
      Real*8  xStep(3), t1, t2
      Real*8  calVol, calSqr, heit, v, s
...
C group (Local variables)
      ...
      Real*8  calVol, v1, vv, mutualOrientation
...
c              v1 = calVol(XYP(1, iPo), XYD, XYC, XYP(1, iPa))
c              v2 = calVol(XYP(1, iPo), XYD, XYC, XYP(1, iPb))
c              If(v1 * v2.GT.0D0) Goto 20

               vv = mutualOrientation(XYP(1, iPo), XYD, XYC, 
     &                                XYP(1, iPa), XYP(1, iPb))
               If(vv.GT.0D0) Goto 20
