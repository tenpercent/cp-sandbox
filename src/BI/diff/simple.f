...
c ... dlya gradienta
      ...
      Real*8 Escl(ntmax),Jscl(ntmax),SenScl(ntmax) !scalar U,E,J,Sens
...
c     mark the Dirichlet point near target point PT
     ...
     &                (vrt(3,n)-PT(3))**2)
      ...
      End do
      labelP(DPid) = 111
      write(*,*) 'Fix point: ', DPid, ', dist = ', DPdist
      write(*,*) vrt(1,DPid), vrt(2,DPid), vrt(3,DPid)
...
C       Call BilinearFormTemplate(
C      &     nv, nb, nt, vrt, labelP, bnd, labelF, tet, material,
C      &     FEM3Dext, DATAFEMI, status,
C      &     nvmax, namax, IA2, JA2, A2, RHS2, nRow2, nCol2,
C      &     MaxWi, iW)
...
c ============================================
C        write(*,*) 'constitute the matrix'
C 
C        call Mblock(IA1,IA2,A1,A2,JA1,JA2,
C      & RHS1,RHS2,nRow1,IA,A,JA,RHS)
C 
c      nCol=nRow
...
C ================================================================
      ...
      INFO = LSolver(LSmeth,LSprec, nRow1,IA1,JA1,A1,RHS1,SOL)
...
C             iv3=bnd(3,n)
C 
C         q1=(vrt(1,iv1)-vrt(1,iv2))**2+
C      &     (vrt(2,iv1)-vrt(2,iv2))**2+
C      &     (vrt(3,iv1)-vrt(3,iv2))**2
C 
C         q2=(vrt(1,iv2)-vrt(1,iv3))**2+
C      &     (vrt(2,iv2)-vrt(2,iv3))**2+
C      &     (vrt(3,iv2)-vrt(3,iv3))**2
C 
C         q3=(vrt(1,iv3)-vrt(1,iv1))**2+
C      &     (vrt(2,iv3)-vrt(2,iv1))**2+
C      &     (vrt(3,iv3)-vrt(3,iv1))**2
C 
C         q4=(sqrt(q1)+sqrt(q2)+sqrt(q3))/2
C         q5=sqrt(q4*(q4-sqrt(q1))*(q4-sqrt(q2))*(q4-sqrt(q3)))
C         Stotal(i)=Stotal(i)+q5
C 
C         MesR(i)=MesR(i)+(SOL(iv1)+SOL(iv2)+SOL(iv3))*q5/3
C         MesI(i)=MesI(i)+(SOL(iv1+nRow1)+SOL(iv2+nRow1)+
C      &        SOL(iv3+nRow1))*q5/3
C 
C       End if
C 
C       End do
...
C       Do i=1,Num
C 
C       MesR(i)=MesR(i)/Stotal(i)
C       MesI(i)=MesI(i)/Stotal(i)
C 
C       End do
C 
C 
C c ... raschet complex napejenii(pr,pi)=raznica srednih potencialov pod elktrodami
C c ... impedancov (real and imaginary part)
C 
C       Do n=1, Num
C 
C       Do i=1, Num
C 
C       SilaToka= (J7**2+J8**2)
C 
C       if (n.LT.i) then
C       pr=MesR(n)-MesR(i)
C       pi=MesI(n)-MesI(i)
C 
C c ... save the naprejenia
C 
C       Open(1,file='napryajeniya.txt')
C         Write(1,*)  n, i, pr, pi
C 
C 
C c ... impedance/absolut values
C 
C       ImpR=dabs((pr*J7+pi*J8))/SilaToka
C       ImpI=dabs((pi*J7-pr*J8))/SilaToka
C 
C       Open(2,file='impedance.txt')
C         Write(2,*) n, i, ImpR, ImpI
C       End if
C 
C       End do
C 
...
C       Close(2)
C 
C       write(*,*) 'file nanapryajeniya.txt is completed'
C       write(*,*) 'file impedance.txt is completed'
C       write(*,*) 'E and J evaluating'
C c ... vycheslenie gradienta
C 
C       Do n = 1,nt
C 
C       av=tet(1,n)
...
C       qw9=XY3(3)-XY4(3)
C 
C       qw=dabs(qw1*qw5*qw9+qw7*qw2*qw6+qw3*qw4*qw8-
C      & qw3*qw5*qw7-qw2*qw4*qw9-qw1*qw8*qw6)/6
C 
C c... real part E
C 
C       SV(1)=SOL(av)/qw
...
C       SV(4)=SOL(dv)/qw
C 
C       Call fem3dtet(XY1, XY2, XY3, XY4,
C      &              GRAD, FEM_P1, IDEN, FEM_P0vector,
C      &              material, Ddiff1, DATAFEME, iSYS, 2,
C      &              4, GV, nr1, nc1)
C 
C 
C       Call dGemv('N',nr1,nc1,1D0,GV,4,SV,1,0d0,RV1,1)
C 
C       Er1(1,n)=RV1(1)
C       Er1(2,n)=RV1(2)
C       Er1(3,n)=RV1(3)
C 
C c... imaginary part E
C 
C       SV2(1)=SOL(av+nrow1)/qw
C       SV2(2)=SOL(bv+nrow1)/qw
C       SV2(3)=SOL(cv+nrow1)/qw
C       SV2(4)=SOL(dv+nrow1)/qw
C 
C       Call dGemv('N',nr1,nc1,1D0,GV,4,SV2,1,0d0,RV2,1)
C 
C       Ei1(1,n)=RV2(1)
C       Ei1(2,n)=RV2(2)
C       Ei1(3,n)=RV2(3)
C 
C       Escl(n)=sqrt(Er1(1,n)**2+Er1(2,n)**2+Er1(3,n)**2+
C      & Ei1(1,n)**2+Ei1(2,n)**2+Ei1(3,n)**2)
C 
C c ... plotnost toka v zavisimosti ot nomera materiala
C 
C       Call Jeval(XY1,XY2,XY3,XY4,material(n),DATAFEMR,DATAFEMI,
C      & RV1,RV2,J1,J2,J3,J4,J5,J6)
C 
C       Jr1(1,n)=J1
C       Jr1(2,n)=J2
C       Jr1(3,n)=J3
...
C       Ji1(3,n)=J6
C 
C       Jscl(n)=sqrt(Jr1(1,n)**2+Jr1(2,n)**2+Jr1(3,n)**2+
C      & Ji1(1,n)**2+Ji1(2,n)**2+Ji1(3,n)**2)
C 
C       End do
C 
C 
C c ... save the matrix
C 
C 	if (chE.EQ.1 .and. chJ.EQ.1) then	
C 
C       Do n=1,nt
C       Open(1,file='E.txt')
C         Write(1,*)  n
C         Write(1,*) (Er1(i,n), i=1,3)
C         Write(1,*) (Ei1(i,n), i=1,3)
C         Write(1,*)  Escl(n)
C        Open(2,file='J.txt')
C         write(2,*)  n
C         Write(2,*) (Jr1(i,n), i=1,3)
C         Write(2,*) (Ji1(i,n), i=1,3)
C 	  Write(2,*)   Jscl(n)
C       End do
C       Close(1)
C       Close(2) 
C 
C       write(*,*) 'files E.txt and J.txt are completed'
C 
C 	Else if (chE.EQ.1 .and. chJ.EQ.0) then
C 	  Do n=1,nt
C         Open(1,file='E.txt')
C         Write(1,*)  n
C         Write(1,*) (Er1(i,n), i=1,3)
...
C       End do
C       Close(1)
C 	write(*,*) 'files E.txt is completed'
C 	Else if (chE.EQ.0 .and. chJ.EQ.1) then
C 	  Do n=1,nt
C         Open(2,file='J.txt')
C         write(2,*)  n
C         Write(2,*) (Jr1(i,n), i=1,3)
...
C       End do
C       Close(2) 
C 	write(*,*) 'files J.txt is completed'
C 	End if
C 
C 
C 
C 
C 	if (chGmv.EQ.1) then
C 
C       call GMVvectorTet(Er1,"er.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C       call GMVvectorTet(Ei1,"ei.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C       call GMVvectorTet(Jr1,"jr.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C       call GMVvectorTet(Ji1,"ji.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C       call GMVscalarTet(Escl,"escl.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C       call GMVscalarTet(Jscl,"jscl.gmv", 10,
C      &  nv, vrt, nt, tet, nb, bnd, labelF)
C 
C 	End if
C 
...
C       DATAFEMI(2)=NU2+10
C 
C c for Neuman conditions vicheslenie ploshadi dlya zond.toka
C       Stotal1=0D0
C       Stotal2=0D0
C 
C       Do n = 1, nb
C       If(labelF(n).EQ.DATAFEMR(1)) then
C             iv1=bnd(1,n)
C             iv2=bnd(2,n)
C             iv3=bnd(3,n)
C 
...
C         q4=(sqrt(q1)+sqrt(q2)+sqrt(q3))/2
C         q5=sqrt(q4*(q4-sqrt(q1))*(q4-sqrt(q2))*(q4-sqrt(q3)))
C         Stotal1=Stotal1+q5
C 
C       else If(labelF(n).EQ.DATAFEMR(2)) then
C             iv1=bnd(1,n)
C             iv2=bnd(2,n)
C             iv3=bnd(3,n)
C 
...
C 
C 
C       DATAFEMR(3)=J7/Stotal1
C       DATAFEMI(3)=J8/Stotal1
C 
C       DATAFEMR(4)=J7/Stotal2
C       DATAFEMI(4)=J8/Stotal2
C 
C       status = IOR(MATRIX_GENERAL, FORMAT_AMG)
C c       for  real part
C 
C          Call BilinearFormTemplate(
C      &     nv, nb, nt, vrt, labelP, bnd, labelF, tet, material,
C      &     FEM3Dext, DATAFEMR, status,
C      &     nvmax, namax, IA1, JA1, A1, RHS1, nRow1, nCol1,
C      &     MaxWi, iW)
C 
C c       for  imaginary part
C 
C       Call BilinearFormTemplate(
C      &     nv, nb, nt, vrt, labelP, bnd, labelF, tet, material,
C      &     FEM3Dext, DATAFEMI, status,
...
C 
C c ============================================
C c zapolnyaem blokami matricu
C c ============================================
C 
C        call Mblock(IA1,IA2,A1,A2,JA1,JA2,
C      & RHS1,RHS2,nRow1,IA,A,JA,RHS)
C 
C C ================================================================
C C  STAGE 2: solve linear system
C C ================================================================
C       nRow=2*nRow1
C 
C       INFO = LSolver(LSmeth,LSprec, nRow,IA,JA,A,RHS,SOL)
C 
C C      if (INFO.ne.0) stop 'Linear solver failed'
C 
C c      call GMVscalarVrtMat(SOL,"mesh.gmv", 10,
C c     &  nv, vrt, nt, tet, material, nb, bnd, labelF)
C 
C 
C 
C 
...
C       End do
C 
C C     save sensitivity for further use
C       Open(1,file='sensitivity.txt')
C 
C       Do n=1,nt
C 
C       Call Sens(n,Jr1,Ji1,Jr2,Ji2,SenR,SenI)
C 
C       SnsR(n)=SenR*1e12/J7/J7
C       SnsI(n)=SenI*1e12/J7/J7
C 
C       SenScl(n)=sqrt(SnsR(n)**2+SnsI(n)**2)
C 
...
C      & qw3*qw5*qw7-qw2*qw4*qw9-qw1*qw8*qw6)/6
C 
C       qw1=(XY1(1)+XY2(1)+XY3(1)+XY4(1))/4
C       qw2=(XY1(2)+XY2(2)+XY3(2)+XY4(2))/4
C       qw3=(XY1(3)+XY2(3)+XY3(3)+XY4(3))/4
C 
C C      h=(qw1+5)/10+24
C C      s=(qw2+5)/10+11
C C      k=(qw3+5)/10+38
C       
C C      SNX(h,k)=SNX(h,k)+qw*SnsR(n)
C C      SNY(s,k)=SNY(s,k)+qw*SnsR(n)
C       st = st + qw*SnsR(n)
C       sm(material(n)) = sm(material(n)) + qw*SnsR(n)
...
C       Close(1)
C 
C       Write(*,*) 'Total sens = ', st
C       Do n=1, 32
C       Write(*,*)'Label',n,' = ',sm(n),'(',sm(n)/st*100d0,'%)'
C       End do
C 
C C      Open(1,file='sens-xz.vtk')
C C      Write(1,'(A)') '# vtk DataFile Version 2.0'
C C      Write(1,'(A)') 'Sensitivity, plane XZ'
C C      Write(1,'(A)') 'ASCII'
C C      Write(1,'(A)') 'DATASET RECTILINEAR_GRID'
C C      Write(1,'(A)') 'DIMENSIONS 47 1 75'
C C      Write(1,'(A)') 'X_COORDINATES 47 float'
C C      Write(1,*) (i*10,i=-23,23)
C C      Write(1,'(A)') 'Y_COORDINATES 1 float'
C C      Write(1,'(A)') '0'
C C      Write(1,'(A)') 'Z_COORDINATES 75 float'
C C      Write(1,*) (i*10,i=-37,37)
C C      Write(1,'(A)') 'POINT_DATA 3525'
C C      Write(1,'(A)') 'SCALARS scalars float'
C C      Write(1,'(A)') 'LOOKUP_TABLE default'
C C      Do i=1,75
C C        Write(1,*)  (SNX(n,i),n=1,47)
C C      End do
C C      Close(1)
C C      
C C      Open(1,file='sens-yz.vtk')
C C      Write(1,'(A)') '# vtk DataFile Version 2.0'
C C      Write(1,'(A)') 'Sensitivity, plane XZ'
C C      Write(1,'(A)') 'ASCII'
C C      Write(1,'(A)') 'DATASET RECTILINEAR_GRID'
C C      Write(1,'(A)') 'DIMENSIONS 1 21 75'
C C      Write(1,'(A)') 'X_COORDINATES 1 float'
C C      Write(1,'(A)') '0'
C C      Write(1,'(A)') 'Y_COORDINATES 21 float'
C C      Write(1,*) (i*10,i=-10,10)
C C      Write(1,'(A)') 'Z_COORDINATES 75 float'
C C      Write(1,*) (i*10,i=-37,37)
C C      Write(1,'(A)') 'POINT_DATA 1575'
C C      Write(1,'(A)') 'SCALARS scalars float'
C C      Write(1,'(A)') 'LOOKUP_TABLE default'
C C      Do i=1,75
C C         Write(1,*)  (SNY(n,i),n=1,21)
C C      End do
C C      Close(1)
C       
C 
C 	if (chVtk.EQ.1) then
C c     save VTK file for ParaView
C       call VTKSpecial("mesh2.vtk", 10,  "output",
C      &  nv, vrt, nt, tet, material, nb, bnd, labelF,
C      &  SOL, "U",
C      &  SOL, "U_Re",
C      &  SOL, "U_Im",
